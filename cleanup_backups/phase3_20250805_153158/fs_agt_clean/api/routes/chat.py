...
from fs_agt_clean.core.websocket.events import AutonomousAgentType, SenderType
...
async def get_chat_root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "websocket": "/ws/flipsync (unified endpoint)",
        },
        "features": [
            "Real-time messaging",
...
async def get_chat_service(request: Request) -> StrategicChatAdapter:
    """Get strategic chat service adapter from app state."""
    chat_service = getattr(request.app.state, "chat_service", None)
    if chat_service is None:
        # Create strategic chat service and adapter
...
async def get_auth_service(request: Request) -> Optional[Any]:
...
async def strategic_chat(
    ...
    chat_service: StrategicChatAdapter = Depends(get_chat_service),
):
    """
    Strategic chat endpoint for direct AI consultation.

    This endpoint provides direct access to the strategic chat service
    for AI-powered business advice and optimization recommendations.
    """
    try:
        logger.info(f"🧠 Strategic chat request: {message_request.text[:50]}...")

        # Generate strategic response
        response = await chat_service.handle_message_enhanced(
            user_id="strategic_user",
            message=message_request.text,
            conversation_id="strategic_session",
            app_context="strategic_consultation",
        )
        ...
        logger.error(f"❌ Strategic chat error: {e}")
        raise HTTPException(status_code=500, detail=f"Strategic chat failed: {str(e)}")
...
async def process_message_with_workflow_detection(
    ...
    conversation_id: str,
    user_message: str,
    chat_service: StrategicChatAdapter,
...
async def process_agent_response_enhanced(
    ...
    try:
        ...
        agent_type_enum = AutonomousAgentType.ASSISTANT
        try:
            agent_type_enum = AutonomousAgentType(agent_type_str)
        except ValueError:
            pass
...
# All WebSocket functionality has been consolidated into /ws/flipsync
# This provides unified authentication and message routing
#
# Previous endpoints:
# - /conversations/{conversation_id}/ws
# - /api/v1/ws/chat/{conversation_id}
# - /api/v1/ws/system
# New unified endpoint: /ws/flipsync
#
# This consolidation eliminates connection conflicts and ensures all WebSocket
# communication goes through a single, optimized endpoint.
