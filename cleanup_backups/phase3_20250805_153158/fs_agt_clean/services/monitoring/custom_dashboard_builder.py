...
class CustomDashboardBuilder:
    ...
    async def _fetch_system_metrics(self, query: str) -> Any:
        ...
        # Mock data removed - requires real system monitoring integration
        raise NotImplementedError(
            "System metrics require real monitoring integration. "
            "Mock data has been removed to prevent conflicts."
        )

    async def _fetch_business_metrics(self, query: str) -> Any:
        """Fetch business metrics data."""
        # Mock data removed - requires real eBay integration
        raise NotImplementedError(
            "Business metrics require real eBay integration. "
            "Mock data has been removed to prevent conflicts."
        )

    async def _fetch_agent_metrics(self, query: str) -> Any:
        """Fetch real agent metrics data from agent manager."""
        try:
            # FIXED: Connect to real agent manager instead of mock data
            from fs_agt_clean.core.agents.real_agent_manager import (
                RealAutonomousAgentManager,
            )

            # Get agent manager instance (in production, this would be injected)
            agent_manager = RealAutonomousAgentManager()

            # Get real agent status
            agent_statuses = await agent_manager.get_all_agent_statuses()
            ...
            for agent_id, status_info in agent_statuses.items():
                ...
                if total_operations > 0:
                    performance = (success_count / total_operations) * 100
                else:
                    performance = 100.0  # No operations yet, assume healthy
            ...
            logger.error(f"Error fetching real agent metrics: {e}")
            # Fallback to basic status if real data unavailable
            return [
                {
                    "agent_id": "agent_manager_unavailable",
                    "status": "error",
                    "performance": 0.0,
                    "error": str(e),
                }
        ...
        async def update_loop():
            while True:
                try:
                    ...
                    await self.get_widget_data(dashboard_id, widget_id)
