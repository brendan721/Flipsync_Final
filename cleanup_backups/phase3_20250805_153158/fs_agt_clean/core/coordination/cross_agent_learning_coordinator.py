...
# Legacy import removed - using 4+1 architecture models only
from fs_agt_clean.database.models.autonomous_agent import AutonomousAgentCommunication
...
class CrossAgentLearningCoordinator:
    ...
    async def share_learning_insight(
        ...
        try:
            ...
            async with self.database.get_session() as session:
                # Check if source agent is autonomous agent
                from fs_agt_clean.database.repositories.autonomous_agent_repository import (
                    AutonomousAgentRepository,
                )

                autonomous_repo = AutonomousAgentRepository()
                autonomous_agent = await autonomous_repo.get_autonomous_agent(
                    session, source_agent_id
                )

                for target_agent_id in target_agent_ids:
                    if autonomous_agent:
                        # Use new autonomous agent communication table
                        communication_record = AutonomousAgentCommunication(
                            agent_id=autonomous_agent.id,  # Use database ID
                        ...
                        # Use 4+1 architecture communication table for all agents
                        communication_record = AutonomousAgentCommunication(
        ...
        try:
            async with self.database.get_session() as session:
                query = (
                    ...
                    .where(
                        AutonomousAgentCommunication.target_agent_id == agent_id,
                        AutonomousAgentCommunication.message_type
        ...
        try:
            async with self.database.get_session() as session:
                ...
                result = await session.execute(
                    select(AutonomousAgentCommunication)
                    .where(AutonomousAgentCommunication.id == insight_id)
                    .where(AutonomousAgentCommunication.target_agent_id == agent_id)
                    .where(
                        AutonomousAgentCommunication.message_type
                        == "learning_insight_sharing"
                    )
                ...
                await session.execute(
                    update(AutonomousAgentCommunication)
                    .where(AutonomousAgentCommunication.id == insight_id)
                    .where(AutonomousAgentCommunication.target_agent_id == agent_id)
                    .values(
        ...
        try:
            async with self.database.get_session() as session:
                await session.execute(
                    update(AutonomousAgentCommunication)
                    .where(
                        AutonomousAgentCommunication.id == communication_id,
                        AutonomousAgentCommunication.target_agent_id == agent_id,
                    )
                    .values(
                        responded_at=datetime.now(timezone.utc),
        ...
        try:
            async with self.database.get_session() as session:
                ...
                insight_sharing_result = await session.execute(
                    select(AutonomousAgentCommunication).where(
                        AutonomousAgentCommunication.message_type
                        == "learning_insight_sharing"
                    )
                )
    ...
    async def _store_conflict_resolution(
        ...
        try:
            async with self.database.get_session() as session:
                conflict_communication = AutonomousAgentCommunication(
