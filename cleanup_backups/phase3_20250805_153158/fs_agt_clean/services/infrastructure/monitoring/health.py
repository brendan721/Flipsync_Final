...
class HealthMonitor:
    ...
    def __init__(
        ...
        self.agent_health: Dict[str, AutonomousAgentHealth] = {}
    ...
    async def check_health(self, agent_id: str) -> AutonomousAgentHealth:
    ...
    async def get_recovery_action(self, health: AutonomousAgentHealth) -> RecoveryAction:
    ...
    async def create_alert(
        self, health: AutonomousAgentHealth, severity: AlertSeverity, message: str
    ) -> HealthAlert:
        """Create health alert."""
        alert = HealthAlert(
            id=str(uuid4()),
    ...
    async def _check_alerts(self, health: AutonomousAgentHealth) -> None:
        """Check and create alerts based on health status."""
        if health.status != HealthStatus.HEALTHY:
            await self.create_alert(
                health,
                AlertSeverity.HIGH,
                f"AutonomousAgent {health.agent_id} is in {health.status.value} state",
            )
