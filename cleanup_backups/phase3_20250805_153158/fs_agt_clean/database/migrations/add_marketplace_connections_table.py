...
def get_database_url():
    """Get database URL from environment or use correct test database as fallback."""
    return os.getenv(
        "DATABASE_URL",
        "postgresql://postgres:your_password@localhost:5432/flipsync_db",
    )
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            if not marketplaces_exists:
                logger.warning(
                    "⚠️  marketplaces table does not exist. Creating without foreign key constraint."
                )
