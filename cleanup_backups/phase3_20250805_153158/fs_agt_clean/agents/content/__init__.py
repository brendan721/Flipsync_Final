"""
Content Autonomous Agent Services.
...
Note: Uses 4+1 architecture with ContentAutonomousAgent as the main implementation.
"""
...
# Import content agent components (4+1 Architecture)
try:
    from .content_agent_service import ContentAutonomousAgentService
except ImportError:
    ContentAutonomousAgentService = None

try:
    from .content_agent import ContentAutonomousAgent

    # Backward compatibility alias - DEPRECATED
    ContentAutonomousAgent = ContentAutonomousAgent
except ImportError:
    ContentAutonomousAgent = None
    ContentAutonomousAgent = None
...
class ContentAutonomousAgentManager:
    ...
    def __init__(self, config_manager=None, log_manager=None, llm_service=None):
        ...
        self.content_agent = (
            ContentAutonomousAgentService(
                config=config_manager, log_manager=log_manager, llm_service=llm_service
            )
            if ContentAutonomousAgentService and config_manager and log_manager
            else None
        )
