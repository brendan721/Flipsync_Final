...
class AdvancedMultiAgentCoordinator:
    ...
    async def register_agent(
        ...
        try:
            ...
            agent_type = UnifiedAgentCategoryType.MARKET
            if "executive" in agent_id.lower():
                agent_type = UnifiedAgentCategoryType.EXECUTIVE
            elif "content" in agent_id.lower():
                agent_type = UnifiedAgentCategoryType.CONTENT
            elif "logistics" in agent_id.lower():
                agent_type = UnifiedAgentCategoryType.LOGISTICS
    ...
    async def register_agent_with_capabilities(
        self,
        agent_id: str,
        agent_type: UnifiedAgentCategoryType,
        ...
        try:
            if conflict.resolution_method == ConflictResolutionMethod.PRIORITY_BASED:
                ...
                executive_agents = [
                    agent_id
                    for agent_id, info in self.registered_agents.items()
                    if info.get("agent_type") == UnifiedAgentCategoryType.EXECUTIVE
                ]
                if executive_agents:
                    conflict.resolution = {
