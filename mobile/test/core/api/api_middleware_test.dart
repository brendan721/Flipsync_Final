...
void main() {
  ...
  setUpAll(() {
    setupTestDependencies(); // Setup GetIt dependencies
  ...
  group('ApiMiddleware Tests', () {
    ...
    late MockTokenRotationHandler tokenRotationHandler;
    ...
    setUp(() {
      ...
      middleware = ApiMiddleware(dio, authState, csrfInterceptor, tokenRotationHandler: tokenRotationHandler);
      ...
      // Setup default mock behaviors to prevent exceptions
      when(() => authState.isAuthenticated).thenReturn(false);
      when(() => authState.tokenExpired).thenReturn(false);
      when(() => authState.authToken).thenReturn(null);
      when(() => tokenRotationHandler.checkAndPerformRotation()).thenAnswer((_) async => false);
    });
