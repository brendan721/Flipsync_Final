...
class MarketplaceConnectionModel(Base):
    ...
    def credentials(self) -> Dict[str, Any]:
        """Get credentials dictionary combining token fields and metadata.

        This property provides compatibility with the existing API code that
        expects marketplace.credentials.get() access pattern.

        Returns:
            Dict[str, Any]: Combined credentials dictionary
        """
        # Start with connection metadata (contains client_id, scopes, etc.)
        creds = dict(self.connection_metadata) if self.connection_metadata else {}
        ...
        creds.update(
            {
                ...
                "token_expiry": (
                    int(self.token_expires_at.timestamp())
