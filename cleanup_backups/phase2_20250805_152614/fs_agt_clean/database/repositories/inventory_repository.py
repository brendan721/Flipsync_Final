...
class InventoryRepository:
    ...
    def _item_to_dict(self, item: InventoryItem) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_sync_at": (
                item.last_sync_at.isoformat() if item.last_sync_at else None
            ),
        }

    # eBay-specific methods
    async def upsert_ebay_inventory_items(
        self, items_data: List[Dict[str, Any]]
    ) -> List[InventoryItem]:
        """Upsert multiple eBay inventory items (insert or update if exists).

        Args:
            items_data: List of inventory item data dictionaries

        Returns:
            List of created/updated InventoryItem objects
        """
        try:
            upserted_items = []
            ...
            for item_data in items_data:
                ...
                stmt = insert(InventoryItem).values(**item_data)
                stmt = stmt.on_conflict_do_update(
                    ...
                    set_={
                        ...
                        "condition_ebay": stmt.excluded.condition_ebay,
                        "marketplace_source": stmt.excluded.marketplace_source,
            ...
            await self.db_session.rollback()
            logger.error(f"Error upserting eBay inventory items: {e}")
            raise

    async def get_ebay_inventory_items(
        self, limit: int = 100, offset: int = 0
    ) -> List[InventoryItem]:
        """Get inventory items from eBay marketplace.
        ...
        Returns:
            List of eBay inventory items
        """
        try:
            result = await self.db_session.execute(
                select(InventoryItem)
                .where(InventoryItem.marketplace_source == "eBay")
                .order_by(InventoryItem.last_sync_at.desc())
                .limit(limit)
                .offset(offset)
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error getting eBay inventory items: {e}")
            return []

    async def get_stale_ebay_items(
        self, hours_threshold: int = 24
    ) -> List[InventoryItem]:
        """Get eBay inventory items that haven't been synced recently.

        Args:
            hours_threshold: Hours since last sync to consider stale

        Returns:
            List of stale eBay inventory items
        """
        try:
            ...
            result = await self.db_session.execute(
                ...
                .where(
                    and_(
                        InventoryItem.marketplace_source == "eBay",
                        or_(
                            InventoryItem.last_sync_at < cutoff_time,
                            InventoryItem.last_sync_at.is_(None),
                        ),
                    )
                )
                .order_by(InventoryItem.last_sync_at.asc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error getting stale eBay items: {e}")
            return []
