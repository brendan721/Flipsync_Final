...
class UnifiedConfigManager:
    ...
    def __init__(self, config_file: str = ".env.unified"):
        ...
        logger.info(
            f"🔧 Unified Configuration Manager initialized for environment: {self.environment}"
        )
    ...
    def _detect_environment(self) -> str:
        ...
        if (
            os.path.exists("/opt/flipsync")
            or os.getenv("ENVIRONMENT") == "production"
            or "flipsyncai.com" in os.getenv("API_BASE_URL", "")
        ):
            return "production"
    ...
    def _apply_production_config(self):
        ...
        SECURITY NOTE: All sensitive values must be provided via environment variables.
        This method only sets non-sensitive defaults and validates required variables.
        """
        # Non-sensitive production defaults
        production_defaults = {
        ...
        # Apply only non-sensitive defaults if not already set
        for key, value in production_defaults.items():
            if not os.getenv(key):
                os.environ[key] = value

        # Validate that all required sensitive environment variables are set
        required_env_vars = [
            "DB_HOST",
            "DB_PASSWORD",
            "REDIS_HOST",
    ...
    def _apply_staging_config(self):
        ...
        staging_overrides = {
            "API_BASE_URL": "https://staging.flipsyncai.com",
