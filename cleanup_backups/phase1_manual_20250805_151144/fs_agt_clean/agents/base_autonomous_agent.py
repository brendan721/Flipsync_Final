...
class BaseAutonomousAgent(ABC):
    ...
    def __init__(
        ...
        # Initialize 4+1 architecture repository for autonomous agents
    ...
    async def initialize_async(self) -> bool:
        ...
        try:
            ...
            # Register agent in 4+1 architecture database
            await self._register_autonomous_agent()
    ...
    async def _register_autonomous_agent(self) -> None:
        """Register this agent in the 4+1 architecture autonomous_agents table."""
        try:
            ...
            agent_type_mapping = {
                "market": AutonomousAgentType.MARKET,
                "content": AutonomousAgentType.CONTENT,
                "executive": AutonomousAgentType.EXECUTIVE,
                "logistics": AutonomousAgentType.LOGISTICS,
            }

            agent_type_enum = agent_type_mapping.get(self.agent_type)
            if not agent_type_enum:
                raise ValueError(
                    f"Invalid agent type for 4+1 architecture: {self.agent_type}"
                )
            ...
            optimization_config = getattr(self, "optimization_config", {})

            async with self.database.get_session() as session:
                # Register or update agent in autonomous_agents table
                agent = await self.autonomous_agent_repository.create_or_update_autonomous_agent(
                    session=session,
                ...
                logger.info(
                    f"✅ Registered {self.agent_id} in 4+1 architecture (autonomous_agents table)"
                )

                # Store agent database ID for decision tracking
                self._autonomous_agent_db_id = agent.id

        except Exception as e:
            logger.error(
                f"❌ Failed to register autonomous agent in 4+1 architecture: {e}"
            )
            # Continue without registration - agent will work but decisions won't be tracked
            self._autonomous_agent_db_id = None

    def _get_agent_capabilities(self) -> List[str]:
        """Get list of agent capabilities based on agent type."""
        ...
        return capabilities_mapping.get(
            self.agent_type, ["general_autonomous_capability"]
        )
    ...
    async def _record_autonomous_decision(
        ...
        """Record autonomous decision in 4+1 architecture database."""
        try:
            if (
                not hasattr(self, "_autonomous_agent_db_id")
                or not self._autonomous_agent_db_id
            ):
                logger.warning(
                    f"Agent {self.agent_id} not registered in 4+1 architecture - skipping decision recording"
                )
                return

            from fs_agt_clean.database.models.autonomous_agent import DecisionStatus

            async with self.database.get_session() as session:
                # Record decision using autonomous agent repository
                await self.autonomous_agent_repository.record_autonomous_decision(
                    session=session,
                    agent_id=self._autonomous_agent_db_id,  # Use database ID, not agent_id
                ...
                logger.debug(f"✅ Recorded decision {decision_id} in 4+1 architecture")

        except Exception as e:
            logger.error(
                f"❌ Failed to record autonomous decision in 4+1 architecture: {e}"
            )
            # Continue without recording - agent will work but decisions won't be tracked

    async def update_heartbeat(self) -> None:
        """Update agent heartbeat in 4+1 architecture database."""
        try:
            if (
                not hasattr(self, "_autonomous_agent_db_id")
                or not self._autonomous_agent_db_id
            ...
            async with self.database.get_session() as session:
                # Update heartbeat using autonomous agent repository
                success = await self.autonomous_agent_repository.update_agent_heartbeat(
                    session=session,
                    agent_id=self.agent_id,  # Use agent_id for heartbeat lookup
                )

                if success:
                    logger.debug(f"✅ Updated heartbeat for {self.agent_id}")
                else:
                    logger.warning(f"⚠️ Failed to update heartbeat for {self.agent_id}")

        except Exception as e:
            logger.error(f"❌ Failed to update agent heartbeat: {e}")
            # Continue without heartbeat update

    def _get_algorithm_name(self, decision_type) -> str:
        """Get algorithm name for decision type."""
        try:
            # Get algorithm from optimization config based on decision type
            if hasattr(self, "optimization_config") and self.optimization_config:
                # Map decision types to algorithm config keys
                algorithm_mapping = {
                    "optimization": "default_algorithm",
                    "pricing": "pricing_algorithm",
                    "content": "content_generation_algorithm",
                    "logistics": "route_optimization_algorithm",
                    "executive": "resource_allocation_algorithm",
                }

                algorithm_key = algorithm_mapping.get(
                    str(decision_type).lower(), "default_algorithm"
                )
                return self.optimization_config.get(
                    algorithm_key, "standard_decision_pipeline"
                )

            return "standard_decision_pipeline"
        except Exception:
            return "standard_decision_pipeline"
    ...
    async def make_autonomous_decision(
        ...
        try:
            ...
            # Record decision in 4+1 architecture database
            await self._record_autonomous_decision(
                decision_id=decision_id,
