...
class MetricsService:
    ...
    async def store_agent_metrics(
        ...
        """
        async with self.database.get_session_context() as session:
            metrics = AutonomousAgentMetrics(
                agent_id=agent_id,
                timestamp=timestamp or datetime.now(timezone.utc),
                status=status,
    ...
    async def get_agent_metrics_history(
        ...
        """
        async with self.database.get_session_context() as session:
            query = select(AutonomousAgentMetrics)

            conditions = []
            if agent_id:
                conditions.append(AutonomousAgentMetrics.agent_id == agent_id)
            if start_time:
                conditions.append(AutonomousAgentMetrics.timestamp >= start_time)
            if end_time:
                conditions.append(AutonomousAgentMetrics.timestamp <= end_time)

            if conditions:
                query = query.where(and_(*conditions))

            query = query.order_by(desc(AutonomousAgentMetrics.timestamp)).limit(limit)
    ...
    async def get_metrics_summary(
        ...
        async with self.database.get_session_context() as session:
            ...
            agent_count_query = select(func.count(AutonomousAgentMetrics.id)).where(
                and_(
                    AutonomousAgentMetrics.timestamp >= start_time,
                    AutonomousAgentMetrics.timestamp <= end_time,
                )
            )
            ...
            unique_agents_query = select(
                func.count(func.distinct(AutonomousAgentMetrics.agent_id))
            ).where(
                and_(
                    AutonomousAgentMetrics.timestamp >= start_time,
                    AutonomousAgentMetrics.timestamp <= end_time,
