...
from fs_agt_clean.core.auth.unified_auth_system import UnifiedAuthSystem
...
class AccountManager:
    ...
    def __init__(
        ...
        Args:
            ...
            auth_system: Unified authentication system
    ...
    async def add_user(
        self, seller_id: str, email: str, role: UnifiedUserRole
    ) -> UnifiedUserAccount:
        ...
        # Create user with unified auth system
        auth_user = await self.auth_system.authenticate_user(email, "temp_password")
        if auth_user:
            token_info = await self.auth_system.create_tokens(auth_user)
        else:
            # For market agents, we may need to create a service token instead
            logger.warning(
                f"Could not authenticate user {email} for market agent access"
            )
            token_info = None
        user = UnifiedUserAccount(
            ...
            api_key=token_info.access_token if token_info else "no_token",
...
class BaseMarketAutonomousAgent:
