...
class ChatRepository(BaseRepository):
    ...
    async def get_conversation(
        ...
        Args:
            session: Database session
            conversation_id: ID of the conversation (UUID or string)
        ...
        try:
            ...
            query = select(Conversation).where(Conversation.id == conversation_uuid)
        except ValueError:
            # If not a valid UUID, look up by title
            query = select(Conversation).where(Conversation.title == conversation_id)
    ...
    async def get_conversation_messages(
        ...
        try:
            ...
            # If not a valid UUID, look up by conversation title or metadata
            # First try to find a conversation with this string as title
    ...
    async def get_conversation_stats(
        ...
        try:
            ...
            # If not a valid UUID, look up conversation by title
            conversation_query = select(Conversation).where(
                Conversation.title == conversation_id
            )
            conversation_result = await session.execute(conversation_query)
            ...
            if not conversation:
                return {"message_count": 0, "last_message_timestamp": None}
