...
class DatabaseOptimizer:
    ...
    async def _get_cache_hit_ratio(self) -> Dict:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                return {
                    'buffer_cache_hit_ratio': float(cache_hit['ratio']) if cache_hit['ratio'] else 0,
                    'index_cache_hit_ratio': float(index_hit['ratio']) if index_hit['ratio'] else 0
                }
        except Exception as e:
            ...
            async with self.pool.acquire() as conn:
                ...
                for index in indexes:
                    ...
                    try:
                        ...
                        logger.info(f"REINDEX completed for {index_name} in {duration:.2f}s")
                    except Exception as e:
                        results.append({
                            'index': index_name,
                            'status': 'error',
    ...
    async def _optimize_connection_pool(self) -> Dict:
        ...
        try:
            ...
            pool_stats = {
                ...
                'available_connections': len([h for h in self.pool._holders if h._con is not None])
            }
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        if buffer_ratio < 95:
            recommendations.append(f"Buffer cache hit ratio is {buffer_ratio:.1f}% - consider increasing shared_buffers")
        ...
        if 'connection_states' in conn_stats:
            ...
            if active_conns > DB_CONFIG['max_size'] * 0.8:
                recommendations.append("High connection usage - consider increasing connection pool size")
        ...
        if isinstance(index_usage, dict) and 'unused' in index_usage:
            ...
            if unused_count > 0:
                recommendations.append(f"Found {unused_count} unused indexes - consider dropping them")
        ...
        if 'largest_tables' in db_size:
            for table in db_size['largest_tables'][:3]:  # Top 3 largest tables
                if table['size_bytes'] > 1000000000:  # > 1GB
                    recommendations.append(f"Large table {table['tablename']} ({table['size']}) - consider partitioning")
