"""
FlipSync AutonomousAgent Alert System
...
from fs_agt_clean.core.monitoring.agent_monitor import AutonomousAgentEvent, AutonomousAgentHealthStatus
...
class HealthStatusAlertCondition(AlertCondition):
    ...
    def __init__(
        ...
        self.status_triggers = status_triggers or [
            AutonomousAgentHealthStatus.STATUS_DEGRADED,
            AutonomousAgentHealthStatus.STATUS_UNHEALTHY,
        ]
...
class EventAlertCondition(AlertCondition):
    ...
    def __init__(
        ...
        self.event_levels = event_levels or [
            AutonomousAgentEvent.LEVEL_ERROR,
            AutonomousAgentEvent.LEVEL_CRITICAL,
        ]
...
<html>
<body>
<h2>FlipSync AutonomousAgent Alert</h2>
<p><strong>Severity:</strong> {alert['severity']}</p>
<p><strong>Condition:</strong> {alert['condition_name']}</p>
<p><strong>Description:</strong> {alert['description']}</p>
<p><strong>AutonomousAgent:</strong> {alert['agent_id']}</p>
    ...
    def create_default_config(self) -> None:
        ...
        self.add_health_condition(
            HealthStatusAlertCondition(
                ...
                description="AutonomousAgent health is degraded",
                severity=AlertCondition.SEVERITY_WARNING,
                status_triggers=[AutonomousAgentHealthStatus.STATUS_DEGRADED],
                cooldown_seconds=1800,  # 30 minutes
            )
        )
        ...
        self.add_health_condition(
            HealthStatusAlertCondition(
                ...
                description="AutonomousAgent is unhealthy",
                severity=AlertCondition.SEVERITY_CRITICAL,
                status_triggers=[AutonomousAgentHealthStatus.STATUS_UNHEALTHY],
                cooldown_seconds=900,  # 15 minutes
            )
        )
        ...
        self.add_event_condition(
            EventAlertCondition(
                name="error_events",
                description="AutonomousAgent encountered an error",
                severity=AlertCondition.SEVERITY_ERROR,
                event_levels=[AutonomousAgentEvent.LEVEL_ERROR],
                cooldown_seconds=1800,  # 30 minutes
            )
        )
        ...
        self.add_event_condition(
            EventAlertCondition(
                ...
                description="AutonomousAgent encountered a critical error",
                severity=AlertCondition.SEVERITY_CRITICAL,
                event_levels=[AutonomousAgentEvent.LEVEL_CRITICAL],
                cooldown_seconds=900,  # 15 minutes
            )
        )
