...
class ListingContentAutonomousAgent(BaseContentAutonomousAgent):
    """
    ...
    async def _handle_generation_event(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if content_type == "title":
                await self.generate_optimized_title(product_data)
            elif content_type == "description":
                await self.generate_product_description(product_data)
            elif content_type == "bullet_points":
                await self.generate_bullet_points(product_data)
            else:
                await self.generate_complete_listing(product_data)
