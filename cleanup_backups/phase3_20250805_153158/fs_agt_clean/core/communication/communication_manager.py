"""
Communication Manager for FlipSync AutonomousAgent Communication Protocol.

This module provides the high-level interface for agent communication,
integrating the message router with the Pipeline Controller and AutonomousAgent Manager.
"""
...
from fs_agt_clean.core.communication.agent_message_router import AutonomousAgentMessageRouter
...
class CommunicationManager:
    ...
    This manager coordinates between the Pipeline Controller, AutonomousAgent Manager,
    ...
    def __init__(self, agent_manager=None, pipeline_controller=None):
        ...
        self.message_router = AutonomousAgentMessageRouter(agent_manager)
    ...
    def _create_agent_message_handler(self, agent_id: str, agent_info: Dict[str, Any]) -> Callable:
        ...
        async def handle_message(message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
            try:
                logger.info(f"AutonomousAgent {agent_id} received message from {message.sender_id} (type: {message.message_type.value})")
    ...
    async def _handle_command_message(self, agent_id: str, agent_instance, message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
    ...
    async def _handle_query_message(self, agent_id: str, agent_instance, message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
    ...
    async def _handle_update_message(self, agent_id: str, agent_instance, message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
        """Handle an update message for an agent."""
        try:
            # Update messages typically don't require responses
            # Just log the update for now
            logger.info(f"AutonomousAgent {agent_id} received update: {message.content}")
            return None
    ...
    async def _handle_alert_message(self, agent_id: str, agent_instance, message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
        """Handle an alert message for an agent."""
        try:
            # Alert messages may require acknowledgment
            logger.warning(f"AutonomousAgent {agent_id} received alert: {message.content}")
    ...
    async def send_agent_message(self, message: AutonomousAgentMessage) -> bool:
        """Send a message between agents."""
        return await self.message_router.send_message(message)

    async def broadcast_to_category(self, message: AutonomousAgentMessage, category: AutonomousAgentCategoryType) -> int:
        """Broadcast a message to all agents in a specific category."""
        category_mapping = {
            AutonomousAgentCategoryType.MARKET: "market",
            AutonomousAgentCategoryType.EXECUTIVE: "executive", 
            AutonomousAgentCategoryType.CONTENT: "content",
            AutonomousAgentCategoryType.LOGISTICS: "logistics",
            AutonomousAgentCategoryType.SYSTEM: "system",
        }
