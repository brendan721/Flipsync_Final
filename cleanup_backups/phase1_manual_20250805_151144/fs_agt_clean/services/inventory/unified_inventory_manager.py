...
class UnifiedInventoryManager:
    ...
    async def _get_inventory_items_by_sku(self, sku: str) -> List[Dict[str, Any]]:
        """Get inventory items for a specific SKU using real inventory service."""
        try:
            # Use the real inventory service to get items by SKU
            ...
            database = get_database()
            inventory_service = InventoryManagementService(database)

            # Get items for the current user (would need user context in real implementation)
            items = await inventory_service.get_inventory_items(
                seller_id="current_user",  # This should come from context
                sku=sku,
                limit=100,
            )
            return items
        except Exception as e:
            logger.warning(f"Failed to get real inventory for SKU {sku}: {e}")
            return []  # Return empty list instead of mock data

    async def _get_all_inventory_items(self) -> List[Dict[str, Any]]:
        """Get all inventory items using real inventory service."""
        try:
            # Use the real inventory service to get all items
            from fs_agt_clean.services.inventory.service import (
                InventoryManagementService,
            )
            from fs_agt_clean.core.db.database import get_database
            ...
            # Get all items for the current user (would need user context in real implementation)
            items = await inventory_service.get_inventory_items(
                seller_id="current_user", limit=1000  # This should come from context
            )
            return items
        except Exception as e:
            logger.warning(f"Failed to get real inventory items: {e}")
            return []  # Return empty list instead of mock data
