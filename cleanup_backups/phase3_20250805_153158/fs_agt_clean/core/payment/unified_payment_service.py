...
class UnifiedPaymentService:
        ...
        try:
            ...
            if payment.create():
                ...
                await self.metrics_service.track_error(
                    "payment_creation_failed", str(payment.error)
                )
        ...
        try:
            ...
            if billing_plan.create():
                ...
                await self.metrics_service.increment_counter(
                    "subscription_plans_created"
                )
                return {"plan_id": billing_plan.id, "status": "active"}
            else:
                await self.metrics_service.track_error(
                    "plan_creation_failed", str(billing_plan.error)
                )
    ...
    async def create_subscription(
        ...
        try:
            ...
            frequency = self._convert_billing_cycle_to_frequency(
            ...
            if billing_agreement.create():
                ...
                await self.metrics_service.track_error(
                    "subscription_creation_failed", str(billing_agreement.error)
                )
                raise Exception(
                    f"Subscription creation failed: {billing_agreement.error}"
                )

        except Exception as e:
            await self.metrics_service.track_error(
                "subscription_creation_error", str(e)
            )
    ...
    async def cancel_subscription(
        ...
        try:
            ...
            if billing_agreement.cancel({"note": reason}):
                ...
                # Send notification
                await self.notification_service.send_notification(
                    user_id, "Your FlipSync subscription has been cancelled."
                )
                ...
                await self.metrics_service.track_error(
                    "subscription_cancellation_failed", str(billing_agreement.error)
                )
                raise Exception(
                    f"Subscription cancellation failed: {billing_agreement.error}"
                )
    ...
    async def get_subscription_status(self, subscription_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "plan_id": (
                    billing_agreement.plan.id if billing_agreement.plan else None
                ),
            }
        except Exception as e:
