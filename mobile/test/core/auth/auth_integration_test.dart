...
void main() {
  ...
  setUp(() {
    ...
    // Setup basic secure storage behavior with actual storage simulation
  ...
  group('Authentication Integration Tests', () {
    test('Full authentication flow with token refresh', () async {
      ...
      // Wait a bit for the logout .then() callback to complete
    ...
    test('API Middleware handles token refreshing correctly', () async {
      // Arrange: Set up authenticated state with valid token (not expired)
      authState.setAuthState(
        isAuthenticated: true,
        authToken: 'valid_token_123',
        tokenExpiry: DateTime.now().add(const Duration(hours: 1)), // Valid token
      );

      // Set up token rotation handler mock - no rotation needed for valid token
      ...
      when(() => handler.next(any())).thenAnswer((invocation) {
        final processedOptions = invocation.positionalArguments[0] as RequestOptions;
        completer.complete(processedOptions);
      });

      // Process the request through middleware
      middleware.onRequest(options, handler);

      // Wait for the middleware to complete with timeout
      final processedOptions = await completer.future.timeout(
        const Duration(seconds: 5),
        onTimeout: () => throw TimeoutException('Middleware did not complete', const Duration(seconds: 5)),
      );

      // Assert: Token added to headers
      expect(processedOptions.headers['Authorization'], equals('Bearer valid_token_123'));
      expect(processedOptions.headers['Accept'], equals('application/json'));
      expect(processedOptions.headers['Content-Type'], equals('application/json'));

      // Verify token rotation was checked
      verify(() => mockTokenRotationHandler.checkAndPerformRotation()).called(1);
    });
  });
}
