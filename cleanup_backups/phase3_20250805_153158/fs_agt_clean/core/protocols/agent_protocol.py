"""
Protocol definitions for agent communication in the FlipSync AutonomousAgent system.
...
class AutonomousAgentCategoryType(Enum):
...
@dataclass
class UpdateMessage(AutonomousAgentMessage):
...
@dataclass
class AlertMessage(AutonomousAgentMessage):
...
@dataclass
class QueryMessage(AutonomousAgentMessage):
...
@dataclass
class CommandMessage(AutonomousAgentMessage):
...
@dataclass
class ResponseMessage(AutonomousAgentMessage):
...
class BaseAutonomousAgentState:
    """
...
class MarketAutonomousAgentState(BaseAutonomousAgentState):
    """
...
class ExecutiveAutonomousAgentState(BaseAutonomousAgentState):
    """
...
class LogisticsAutonomousAgentState(BaseAutonomousAgentState):
...
class ContentAutonomousAgentState(BaseAutonomousAgentState):
    """
...
class AutonomousAgentMemory:
    """
    AutonomousAgent memory management component that handles different memory types:
...
class AutonomousAgentProtocol(ABC):
    """
    ...
    def __init__(
        self,
        agent_id: str,
        agent_category: AutonomousAgentCategoryType,
        ...
        if agent_category == AutonomousAgentCategoryType.MARKET:
            self.state = MarketAutonomousAgentState()
        elif agent_category == AutonomousAgentCategoryType.EXECUTIVE:
            self.state = ExecutiveAutonomousAgentState()
        elif agent_category == AutonomousAgentCategoryType.LOGISTICS:
            self.state = LogisticsAutonomousAgentState()
        elif agent_category == AutonomousAgentCategoryType.CONTENT:
            self.state = ContentAutonomousAgentState()
        else:
            self.state = BaseAutonomousAgentState()

        self.memory = AutonomousAgentMemory()
        self._event_queue = asyncio.Queue()
        self._running = False
        self._processing_task = None

    @abstractmethod
    async def process_message(self, message: AutonomousAgentMessage) -> Optional[AutonomousAgentMessage]:
        """
    ...
    @abstractmethod
    async def send_message(self, message: AutonomousAgentMessage) -> None:
        """
    ...
    async def start(self) -> None:
        ...
        """
        if self._running:
            logger.warning("AutonomousAgent %s is already running", self.agent_id)
            return
    ...
    async def stop(self) -> None:
        ...
        if not self._running:
            logger.warning("AutonomousAgent %s is not running", self.agent_id)
            return
    ...
    async def queue_message(self, message: AutonomousAgentMessage) -> None:
        """
...
def create_legacy_message_format(message: AutonomousAgentMessage) -> Dict[str, Any]:
    """
...
def convert_from_legacy_format(legacy_message: Dict[str, Any]) -> AutonomousAgentMessage:
    """
    ...
    # Create base message
    base_message = AutonomousAgentMessage(
