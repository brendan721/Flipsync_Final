...
class TestAutonomousAgentEndpoints:
    ...
    AGENT_CAPABILITY: AutonomousAgent status, configuration, coordination
    """
    ...
    def test_get_agents_list(self, client):
        """Test getting list of agents - should return exactly 5 agents (4+1 architecture)"""
        response = client.get("/api/v1/agents/list")

        assert response.status_code == status.HTTP_200_OK
        data = response.json()

        # Test for 4+1 architecture compliance
        if isinstance(data, dict) and "agents" in data:
            # New frontend integration format
            agents = data["agents"]
            assert (
                len(agents) == 5
            ), f"Expected exactly 5 agents (4+1 architecture), got {len(agents)}"

            # Verify architecture compliance
            autonomous_count = sum(
                1 for agent in agents if agent.get("type") == "autonomous"
            )
            conversational_count = sum(
                1 for agent in agents if agent.get("type") == "conversational"
            )

            assert (
                autonomous_count == 4
            ), f"Expected 4 autonomous agents, got {autonomous_count}"
            assert (
                conversational_count == 1
            ), f"Expected 1 conversational interface, got {conversational_count}"

            # Verify agent structure for 4+1 architecture
            agent = agents[0]
            required_fields = [
                "id",
                "name",
            ...
            # Legacy format - should still work but expect 5 agents
            assert (
                len(data) == 5
            ), f"Expected exactly 5 agents (4+1 architecture), got {len(data)}"
