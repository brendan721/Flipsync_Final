...
from fs_agt_clean.agents.content.enhanced_image_agent import EnhancedImageAutonomousAgent
...
enhanced_image_agent: Optional[EnhancedImageAutonomousAgent] = None


async def get_enhanced_image_agent() -> EnhancedImageAutonomousAgent:
    """Get or create enhanced image agent instance."""
    global enhanced_image_agent
    if enhanced_image_agent is None:
        enhanced_image_agent = EnhancedImageAutonomousAgent()
...
class AdvancedProcessingRequest(BaseModel):
    ...
    quality_target: float = Field(
        default=0.9, ge=0.0, le=1.0, description="Target quality score"
    )
...
async def analyze_image_enhanced(
    ...
    agent: EnhancedImageAutonomousAgent = Depends(get_enhanced_image_agent),
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Enhanced analysis request: {len(request.analysis_types)} analysis types"
        )
...
async def process_image_advanced(
    ...
    try:
        logger.info(
            f"Advanced processing request: {len(request.operations)} operations"
        )
...
async def get_available_models() -> Dict[str, Any]:
    ...
    try:
        models_info = {
            VisionModelProvider.GEMINI_VISION.value: {
                "name": "Gemini Vision",
                "description": "Cost-effective vision model powered by Google Gemini",
                "cost_tier": "low",
                "recommended_for": [
        ...
        return {
            ...
            "data": {
                ...
                "default_model": VisionModelProvider.GEMINI_VISION.value,
                "fallback_order": [
                    VisionModelProvider.GEMINI_VISION.value,
                    VisionModelProvider.GOOGLE_VISION.value,
                    VisionModelProvider.CLAUDE_VISION.value,
                ],
            },
            "timestamp": datetime.now(timezone.utc).isoformat(),
