...
class Database:
    ...
    async def create_tables(self) -> None:
        ...
        try:
            # Import 4+1 architecture models (autonomous agents)
            from fs_agt_clean.database.models.autonomous_agent import AutonomousAgent

            self.logger.info("Imported AutonomousAgent")
        ...
        try:
            ...
            self.logger.info("Imported 4+1 architecture agent models")
        except Exception as e:
        ...
        # Import 4+1 architecture models
        try:
            from fs_agt_clean.database.models.autonomous_agent import (
                AutonomousAgent,
                ConversationalInterface,
            ...
            self.logger.info("Imported 4+1 architecture models")
        except Exception as e:
            self.logger.error(f"Error importing 4+1 architecture models: {str(e)}")

        # Import learning system models
        try:
            ...
            self.logger.info("Imported learning system models")
        except Exception as e:
            self.logger.error(f"Error importing learning system models: {str(e)}")
        ...
        try:
            ...
            self.logger.info("Imported inventory models")
        except Exception as e:
            self.logger.error(f"Error importing inventory models: {str(e)}")
        ...
        try:
            ...
            self.logger.info("Imported coordination models")
        except Exception as e:
            self.logger.error(f"Error importing coordination models: {str(e)}")
        ...
        try:
            ...
            # Add 4+1 architecture core tables
            try:
                tables.append(AutonomousAgent.__table__)
                self.logger.info("Added AutonomousAgent table to creation list")
            except NameError as e:
                self.logger.warning(f"AutonomousAgent not available: {e}")
            ...
            try:
                tables.extend(
                    [
                        ...
                        AutonomousAgentMetrics.__table__,
            ...
            try:
                tables.extend(
                    [
                        ...
                        AutonomousAgentDecision.__table__,
                        AutonomousAgentCommunication.__table__,
                    ]
                )
                self.logger.info(
                    "Added chat and 4+1 architecture agent tables to creation list"
                )
            except NameError as e:
                self.logger.warning(f"Chat and agent models not available: {e}")

            # Add 4+1 architecture tables if they were imported successfully
            try:
                tables.extend(
                    [
                        AutonomousAgent.__table__,  # Create autonomous agents first (no dependencies)
                        ConversationalInterface.__table__,  # Create conversational interfaces second (no dependencies)
                        AutonomousAgentDecision.__table__,  # Create decisions (depends on autonomous agents)
                        AutonomousAgentCommunication.__table__,  # Create communications (depends on autonomous agents)
                    ]
                )
                self.logger.info("Added 4+1 architecture tables to creation list")
            except NameError as e:
                self.logger.warning(f"4+1 architecture models not available: {e}")

            # Add learning system tables if they were imported successfully
            try:
                learning_tables = [
                    PolicyOptimizationHistory.__table__,  # Policy optimization tracking
                    PolicyStrategyEvolution.__table__,  # Strategy evolution tracking
                    LearningKnowledgeBase.__table__,  # Knowledge base for learning patterns
                    LearningFeedbackHistory.__table__,  # Feedback processing history
                    LearningPerformanceMetrics.__table__,  # Performance metrics over time
                    CrossAgentLearningInsights.__table__,  # Cross-agent learning insights
                    CrossAgentCoordinationState.__table__,  # Cross-agent coordination
                    LearningConflictResolution.__table__,  # Learning conflict resolution
                ]
                tables.extend(learning_tables)
                self.logger.info(
                    f"Added {len(learning_tables)} learning system tables to creation list"
                )
            except NameError as e:
                self.logger.warning(f"Learning system models not available: {e}")

            # Add inventory tables if they were imported successfully
            try:
                tables.extend(
                    [
                        InventoryLocation.__table__,  # Create locations first (no dependencies)
                        InventoryItem.__table__,  # Create items second
                        InventoryTransaction.__table__,  # Create transactions third (depends on items)
                        InventoryAdjustment.__table__,  # Create adjustments fourth (depends on items)
                        InventoryMovement.__table__,  # Create movements last (depends on items and locations)
                    ]
                )
                self.logger.info("Added inventory tables to creation list")
            except NameError as e:
                self.logger.warning(f"Inventory models not available: {e}")

            # Add coordination tables if they were imported successfully
            try:
                tables.extend(
                    [
                        AgentCapabilityRegistry.__table__,  # Create capability registry first (no dependencies)
                        CoordinationState.__table__,  # Create coordination state second
                        WorkflowState.__table__,  # Create workflow state third (depends on coordination state)
                        AgentCommunication.__table__,  # Create coordination communication fourth (depends on coordination and workflow)
                        DistributedDecisionState.__table__,  # Create distributed decision state last
                    ]
                )
                self.logger.info("Added coordination tables to creation list")
            except NameError as e:
                self.logger.warning(f"Coordination models not available: {e}")
