...
async def get_system_metrics(
    request: Request,
    user: UnifiedUserResponse = Depends(require_admin_permission),
):
    """
...
@router.get("/status/public", tags=["monitoring"])
async def get_public_system_status():
    """
    Get basic system status information without authentication.
    This endpoint is designed for external monitoring systems.
    """
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "status": "operational",
...
async def get_detailed_health(
    ...
    try:
        ...
        logger.error("AutonomousAgent system health check failed: %s", str(e))
        response["components"]["agent_system"] = {
            "status": HealthStatus.UNKNOWN.value,
            "message": f"AutonomousAgent system health check failed: {str(e)}",
        }
