...
void main() {
  ...
  group('Authentication Error Handling Tests', () {
    ...
    test('handles invalid token format', () async {
      ...
      when(() => secureStorage.read(key: 'access_token')).thenAnswer((_) async => invalidToken);
      when(() => secureStorage.read(key: 'refresh_token')).thenAnswer((_) async => 'refresh.token');
    ...
    test('handles refresh token expiration', () async {
      ...
      when(() => secureStorage.read(key: 'access_token')).thenAnswer((_) async => 'expired.access.token');
      when(() => secureStorage.read(key: 'refresh_token')).thenAnswer((_) async => testRefreshToken);
    ...
    test('handles corrupted token storage', () async {
      when(() => secureStorage.read(key: 'access_token')).thenAnswer((_) async => 'corrupted_token');
      when(() => secureStorage.read(key: 'refresh_token')).thenAnswer((_) async => 'corrupted_refresh_token');
