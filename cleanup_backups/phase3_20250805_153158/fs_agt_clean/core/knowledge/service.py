...
class KnowledgeSharingService:
    ...
    async def publish_knowledge(
        ...
        logger.info(
            "AutonomousAgent %s published knowledge entry %s: %s",
            source_agent_id,
            entry.id,
            title,
        )
    ...
    async def validate_knowledge(
        ...
        if entry.source_agent_id == validator_agent_id:
            logger.warning(
                "AutonomousAgent %s attempted to validate its own knowledge entry %s",
                validator_agent_id,
                entry_id,
            )
            return False
        ...
        logger.info(
            "AutonomousAgent %s validated knowledge entry %s with score %s",
            validator_agent_id,
            entry_id,
            validation_score,
        )
    ...
    async def subscribe(
        ...
        logger.info(
            "AutonomousAgent %s subscribed to knowledge updates: %s", agent_id, subscription.id
        )
    ...
    async def unsubscribe(self, subscription_id: str) -> bool:
        ...
        logger.info(
            "AutonomousAgent %s unsubscribed from knowledge updates: %s",
    ...
    async def unsubscribe_agent(self, agent_id: str) -> int:
        ...
        logger.info(
            "AutonomousAgent %s unsubscribed from all knowledge updates: %s subscriptions cancelled",
            agent_id,
            count,
        )
