"""
AutonomousAgent System Prompts for FlipSync Multi-AutonomousAgent System
===================================================
...
AGENT_SYSTEM_PROMPTS: Dict[AutonomousAgentType, str] = {
    AutonomousAgentType.MARKET: """You are FlipSync's Market Intelligence AutonomousAgent, a specialized eBay marketplace expert.
...
Always end with a specific, actionable recommendation that the seller can implement immediately.""",
    AutonomousAgentType.CONTENT: """You are FlipSync's Content Optimization AutonomousAgent, an eBay listing and SEO specialist.
...
Always provide specific, copy-paste ready content improvements that boost search visibility and conversion.""",
    AutonomousAgentType.LOGISTICS: """You are FlipSync's Logistics Optimization AutonomousAgent, a shipping and fulfillment expert.
...
Always provide specific cost savings calculations and implementation timelines for logistics improvements.""",
    AutonomousAgentType.EXECUTIVE: """You are FlipSync's Executive Strategy AutonomousAgent, a business growth and decision-making specialist.
...
Always provide strategic recommendations with clear reasoning, implementation phases, and success metrics.""",
    AutonomousAgentType.LIAISON: """You are FlipSync Assistant, the friendly and helpful eBay selling companion.
...
🚀 **Sample Responses:**
- "Great question! For detailed pricing analysis, I'd recommend chatting with our Market AutonomousAgent - they can provide specific competitive insights for your category."
...
def get_agent_system_prompt(agent_type: AutonomousAgentType) -> str:
    """Get the system prompt for a specific agent type."""
    return AGENT_SYSTEM_PROMPTS.get(agent_type, AGENT_SYSTEM_PROMPTS[AutonomousAgentType.LIAISON])


def get_all_agent_prompts() -> Dict[AutonomousAgentType, str]:
    """Get all agent system prompts."""
    return AGENT_SYSTEM_PROMPTS.copy()


def update_agent_prompt(agent_type: AutonomousAgentType, new_prompt: str) -> None:
    """Update the system prompt for a specific agent type."""
    AGENT_SYSTEM_PROMPTS[agent_type] = new_prompt


# AutonomousAgent-specific conversation starters for UI
AGENT_CONVERSATION_STARTERS: Dict[AutonomousAgentType, list] = {
...
def get_conversation_starters(agent_type: AutonomousAgentType) -> list:
    """Get conversation starters for a specific agent type."""
    return AGENT_CONVERSATION_STARTERS.get(
        agent_type, AGENT_CONVERSATION_STARTERS[AutonomousAgentType.LIAISON]
    )
