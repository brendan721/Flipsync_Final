...
class AutonomousAgent(Base):
    ...
    communications: Mapped[List["AutonomousAgentCommunication"]] = relationship(
...
class ConversationalInterface(Base):
    ...
    # LLM provider tracking (Gemini-exclusive for 4+1 architecture)
    llm_provider: Mapped[str] = mapped_column(
        String(100), nullable=False, default="gemini"
    )  # Must be "gemini"
...
class AutonomousAgentCommunication(Base):
    """
    Autonomous Agent Communication Tracking

    Tracks communications between autonomous agents in the 4+1 architecture.
    Replaces legacy agent_communications table for autonomous agents.
    """

    __tablename__ = "autonomous_agent_communications"
    __table_args__ = {"extend_existing": True}
    ...
    # Agent identification (using autonomous_agents table)
    agent_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("autonomous_agents.id"), nullable=False
    )
    ...
    # Foreign key relationship
    agent: Mapped["AutonomousAgent"] = relationship(
        "AutonomousAgent", back_populates="communications"
    )
