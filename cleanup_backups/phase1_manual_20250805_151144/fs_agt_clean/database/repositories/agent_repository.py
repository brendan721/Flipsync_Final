...
class UnifiedAgentRepository(BaseRepository):
    ...
    def __init__(self):
        # Initialize with UnifiedAgent as the primary model
        super().__init__(model_class=UnifiedAgent, table_name="unified_agents")
    ...
    async def create_or_update_agent_status(
        ...
        existing_query = select(UnifiedAgent).where(UnifiedAgent.id == agent_id)
        ...
        if existing_agent:
            ...
            # Note: UnifiedAgent doesn't have metrics/config fields like the old model
            # These would be stored in the configuration field as JSON
            if config:
                existing_agent.configuration = str(config)  # Store as JSON string
            ...
            agent = UnifiedAgent(
                ...
                status=status,
                configuration=str(config) if config else None,
            )
    ...
    async def get_agent_status(
        ...
        Returns:
            UnifiedAgent if found, None otherwise
        """
        query = select(UnifiedAgent).where(UnifiedAgent.id == agent_id)
    ...
    async def get_all_agent_statuses(
        ...
        Returns:
            List of agents
        """
        query = select(UnifiedAgent).order_by(UnifiedAgent.agent_type, UnifiedAgent.id)

        if agent_type:
            query = query.where(UnifiedAgent.agent_type == agent_type)

        if status:
            query = query.where(UnifiedAgent.status == status)
    ...
    async def create_agent_decision(
        ...
        """
        # Serialize parameters to JSON string for database storage
        parameters_json = json.dumps(parameters or {}) if parameters else None
    ...
    async def approve_decision(
        ...
        """
        query = select(UnifiedAgentDecision).where(
            UnifiedAgentDecision.id == uuid.UUID(decision_id)
        )
        result = await session.execute(query)
    ...
    async def get_agent_metrics(
        ...
        if metric_name:
            query = query.where(
                UnifiedAgentPerformanceMetric.metric_name == metric_name
            )
