...
class SoftDeleteMixin:
    ...
    is_deleted: Mapped[bool] = mapped_column(
        default=False, nullable=False, comment="Soft delete flag"
    )
...
class AuditMixin:
    ...
    created_by: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="UnifiedUser who created the record"
    )
    updated_by: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="UnifiedUser who last updated the record"
    )
...
def receive_before_update(_, connection, target):
