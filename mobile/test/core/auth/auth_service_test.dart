...
class AuthTestHelper {
  ...
  static void setupSuccessfulAuth(MockDio mockDio, MockFlutterSecureStorage mockStorage) {
    when(() => mockDio.post(
          '/api/v1/auth/login',
          data: {
            'email': validEmail,
      ...
      (_) async => Response(
        data: {
          ...
          'user': {'id': 'test-user-id', 'email': validEmail},
        },
        statusCode: 200,
  ...
  static void verifyAuthSuccess(MockFlutterSecureStorage mockStorage) {
    verify(() => mockStorage.write(key: 'access_token', value: testAccessToken)).called(1);
    verify(() => mockStorage.write(key: 'refresh_token', value: testRefreshToken)).called(1);
  }
}
  ...
  group('AuthService', () {
    group('login', () {
      test('successful login stores tokens', () async {
        ...
        // Act
        await authService.login(AuthTestHelper.validEmail, AuthTestHelper.validPassword);

        // Assert
        AuthTestHelper.verifyAuthSuccess(mockStorage);
      });

      test('invalid credentials returns failure result', () async {
        // Arrange
        AuthTestHelper.setupFailedAuth(mockDio);

        // Act
        final result = await authService.login(AuthTestHelper.validEmail, 'wrong_password');
    ...
    group('logout', () {
      test('logout clears stored tokens', () async {
        ...
        when(() => mockStorage.read(key: 'access_token')).thenAnswer((_) async => AuthTestHelper.testAccessToken);
        when(() => mockStorage.read(key: 'refresh_token')).thenAnswer((_) async => AuthTestHelper.testRefreshToken);
    ...
    group('token management', () {
      ...
      test('refreshTokens updates stored tokens', () async {
        ...
        when(() => mockStorage.read(key: 'access_token')).thenAnswer((_) async => 'old_access_token');
        when(() => mockStorage.read(key: 'refresh_token')).thenAnswer((_) async => 'old_refresh_token');
