...
class CachedLLMClientFactory:
    ...
    async def create_cached_hybrid_client(
        ...
        # Create HybridLLMAdapter client
        if client_type == "business":
            llm_client = HybridLLMAdapterFactory.create_business_client()
        elif client_type == "fast":
            llm_client = HybridLLMAdapterFactory.create_fast_client()
        elif client_type == "smart":
            llm_client = HybridLLMAdapterFactory.create_smart_client()
        else:
            llm_client = HybridLLMAdapterFactory.create_business_client()

        # Create cached client
        return await CachedLLMClientFactory.create_cached_client(
            llm_client, redis_url, cache_db
        )

    @staticmethod
    def create_non_cached_client(llm_client: HybridLLMClient) -> CachedLLMClient:
        """Create a non-cached LLM client (cache disabled)."""
        return CachedLLMClient(llm_client, None)


# Convenience functions for common use cases - migrated to HybridLLMAdapter
async def create_fast_cached_client() -> CachedLLMClient:
    """Create a fast cached client for simple queries using HybridLLMAdapter."""
    fast_client = HybridLLMAdapterFactory.create_fast_client()
    return await CachedLLMClientFactory.create_cached_client(fast_client)


async def create_smart_cached_client() -> CachedLLMClient:
    """Create a smart cached client for complex queries using HybridLLMAdapter."""
    smart_client = HybridLLMAdapterFactory.create_smart_client()
    return await CachedLLMClientFactory.create_cached_client(smart_client)
