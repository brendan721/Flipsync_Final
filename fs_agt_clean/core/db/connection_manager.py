...
class DatabaseConnectionManager:
    ...
    async def initialize(self) -> bool:
        ...
        for retry in range(self._max_retries + 1):
            try:
                # Create engine with optimized configuration for concurrent agents
                self._engine = create_async_engine(
                    self._connection_string,
                    pool_size=max(
                        self._pool_size, 20
                    ),  # Minimum 20 for concurrent agents
                    max_overflow=max(
                        self._max_overflow, 40
                    ),  # Increased overflow capacity
                    echo=self._echo,
                    pool_pre_ping=True,  # Check connection validity before using
                    pool_recycle=1800,  # Reduced recycle time for fresher connections
                    pool_timeout=10,  # Reduced timeout for faster failure detection
                    connect_args={
                        "command_timeout": 5,  # Reduced command timeout for faster operations
                        "server_settings": {
                            "application_name": "FlipSync_Agentic_System",  # Identify app in pg_stat_activity
                            "jit": "off",  # Disable JIT for consistent performance
                            # Note: shared_preload_libraries cannot be changed without server restart
                        },
                    },
                )
