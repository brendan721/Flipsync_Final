...
class EnhancedWebSocketManager:
    ...
    def __init__(self):
        ...
        # Conversation ID mapping (original_id -> uuid_id)
        self.conversation_id_mapping: Dict[str, str] = {}
    ...
    async def send_to_conversation(
        ...
        """
        CRITICAL FIX: Enhanced conversation broadcasting that automatically tries multiple conversation IDs.
        This ensures frontend receives messages regardless of conversation ID mapping issues.
        """
        logger.info(
            f"🔧 CRITICAL FIX: send_to_conversation called with conversation_id='{conversation_id}'"
        )
        sent_count = 0
        conversation_ids_to_try = [conversation_id]

        # CRITICAL FIX: Check conversation mapping for reverse lookup
        original_id = None
        for orig_id, uuid_id in self.conversation_id_mapping.items():
            if uuid_id == conversation_id:
                original_id = orig_id
                break

        if original_id and original_id not in conversation_ids_to_try:
            conversation_ids_to_try.append(original_id)
            logger.info(
                f"🔧 CRITICAL FIX: Added mapped original ID '{original_id}' for UUID '{conversation_id}'"
            )

        # CRITICAL FIX: Also try "main" if the conversation_id is a UUID
        if conversation_id != "main" and len(conversation_id) > 10:  # Likely a UUID
            conversation_ids_to_try.append("main")
            logger.info(
                f"🔧 CRITICAL FIX: UUID conversation detected, also trying 'main'"
            )
        else:
            logger.info(
                f"🔧 CRITICAL FIX: Not a UUID conversation ('{conversation_id}'), using as-is"
            )

        for conv_id in conversation_ids_to_try:
            if conv_id in self.conversation_connections:
                client_ids = list(self.conversation_connections[conv_id])
                for client_id in client_ids:
                    if await self.send_to_client(client_id, message):
                        sent_count += 1
                logger.info(
                    f"🔧 CRITICAL FIX: Sent to conversation {conv_id}: {len(client_ids)} clients"
                )
            else:
                logger.info(
                    f"🔧 CRITICAL FIX: No clients found for conversation {conv_id}"
                )

        logger.info(f"🔧 CRITICAL FIX: Total recipients: {sent_count}")
        return sent_count

    async def send_to_conversation_smart(
        self,
        conversation_id: str,
        ...
        """
        CRITICAL FIX: Smart conversation broadcasting that sends to both UUID and original conversation IDs.
        This ensures frontend receives messages regardless of conversation ID mapping issues.
        """
        sent_count = 0
        conversation_ids_to_try = [conversation_id]

        # Add original conversation ID if different (for frontend compatibility)
        if original_conversation_id and original_conversation_id != conversation_id:
            conversation_ids_to_try.append(original_conversation_id)

        # Also try common conversation ID patterns
        if conversation_id != "main" and "main" not in conversation_ids_to_try:
            conversation_ids_to_try.append("main")
            ...
            if conv_id in self.conversation_connections:
                ...
                for client_id in client_ids:
                    if await self.send_to_client(client_id, message):
                        sent_count += 1
                logger.info(
                    f"🔄 Sent to conversation {conv_id}: {len(client_ids)} clients"
                )
            else:
                logger.info(f"🔄 No clients found for conversation {conv_id}")

        logger.info(f"🔄 Total smart broadcast recipients: {sent_count}")
        return sent_count

    def register_conversation_mapping(self, original_id: str, uuid_id: str):
        """Register a mapping between original conversation ID and UUID conversation ID."""
        self.conversation_id_mapping[original_id] = uuid_id
        logger.info(
            f"🔧 Registered conversation mapping: '{original_id}' -> '{uuid_id}'"
        )
