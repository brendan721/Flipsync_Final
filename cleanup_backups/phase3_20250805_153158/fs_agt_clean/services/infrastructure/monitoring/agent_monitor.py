"""
FlipSync AutonomousAgent Monitoring System
...
class AutonomousAgentHealthStatus:
...
class AutonomousAgentMonitor:
    ...
    def __init__(self, output_dir: str = "monitoring_data"):
        ...
        self.metrics: Dict[str, List[AutonomousAgentMetric]] = {}  # agent_id -> metrics
        self.events: Dict[str, List[AutonomousAgentEvent]] = {}  # agent_id -> events
        self.health_status: Dict[str, AutonomousAgentHealthStatus] = (
            {}
    ...
    def register_agent(self, agent_id: str) -> None:
        ...
        if agent_id not in self.health_status:
            self.health_status[agent_id] = AutonomousAgentHealthStatus(agent_id)
    ...
    def record_metric(self, name: str, value: Any, agent_id: str) -> None:
        ...
        metric = AutonomousAgentMetric(name, value, agent_id)
        ...
        if isinstance(value, (int, float)) and value > 0:
            logger.debug(f"AutonomousAgent {agent_id}: {name} = {value}")
    ...
    def record_event(
        ...
        message: str,
        agent_id: str,
        level: str = AutonomousAgentEvent.LEVEL_INFO,
        details: Optional[Dict[str, Any]] = None,
    ) -> None:
        ...
        event = AutonomousAgentEvent(event_type, message, agent_id, level, details)
        ...
        if level == AutonomousAgentEvent.LEVEL_WARNING:
            log_func = logger.warning
        elif level == AutonomousAgentEvent.LEVEL_ERROR:
            log_func = logger.error
        elif level == AutonomousAgentEvent.LEVEL_CRITICAL:
            log_func = logger.critical

        log_func(f"AutonomousAgent {agent_id} {event_type}: {message}")
    ...
    def get_health_status(self, agent_id: str) -> Dict[str, Any]:
        ...
        if agent_id not in self.health_status:
            return {
                "agent_id": agent_id,
                "status": AutonomousAgentHealthStatus.STATUS_UNKNOWN,
                "details": {},
    ...
    def _check_health_from_resources(
        ...
        if (
            ...
            status = AutonomousAgentHealthStatus.STATUS_UNHEALTHY
        elif (
            cpu_percent > CPU_WARNING_THRESHOLD or memory_mb > MEMORY_WARNING_THRESHOLD
        ):
            status = AutonomousAgentHealthStatus.STATUS_DEGRADED
        else:
            status = AutonomousAgentHealthStatus.STATUS_HEALTHY
        ...
        if (
            ...
            if status != AutonomousAgentHealthStatus.STATUS_HEALTHY:
                level = (
                    AutonomousAgentEvent.LEVEL_WARNING
                    if status == AutonomousAgentHealthStatus.STATUS_DEGRADED
                    else AutonomousAgentEvent.LEVEL_ERROR
                )
                self.record_event(
...
class AutonomousAgentMonitoringMixin:
    """A mixin to add monitoring capabilities to any agent class."""

    def __init__(self, agent_id: str, monitor: AutonomousAgentMonitor):
        self.agent_id = agent_id
