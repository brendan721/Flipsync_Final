"""
Real-time AutonomousAgent Status Dashboard API routes for FlipSync Frontend Integration.
...
async def get_agent_dashboard_metrics(
    ...
    - AutonomousAgent utilization and performance metrics
    - Task completion statistics
    - Error rates and response times
    """
    try:
        logger.info(
            f"AutonomousAgent dashboard metrics request: type={agent_type}, status={status}"
        )
        ...
        logger.error(f"AutonomousAgent dashboard metrics request failed: {e}")
        return JSONResponse(
            status_code=500,
            content={
                "success": False,
                "error": f"AutonomousAgent metrics request failed: {str(e)}",
            },
        )
...
async def get_workflow_dashboard_metrics(
    ...
    - AutonomousAgent coordination statistics
...
async def get_performance_dashboard_metrics(
    ...
    - AutonomousAgent coordination performance
    ...
    # DISABLED: Legacy WebSocket endpoint - Use unified /ws/flipsync endpoint instead
    # @router.websocket("/ws")
    ...
    # LEGACY: This endpoint has been replaced by the unified /ws/flipsync endpoint
    # Dashboard updates are now handled through the unified WebSocket system
    # """
    pass
